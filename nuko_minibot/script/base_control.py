...
class BaseControl:
    def __init__(self):
        ...
        self.wheelSep = float( rospy.get_param('~wheel_separation', '0.154') ) # unit: meter 
        self.wheelRad = float( rospy.get_param('~wheel_radius', '0.0335') ) # unit: meter
    ...
    def timerOdomCB(self, event):
        ...
        try:           
            ...
            # Normal mode            
            if len(data) == 6:
                WL = float( (data[2]*256 + data[3] -500)*100.0/1560.0*2*math.pi ) # unit: rad/sec
                WR = float( (data[4]*256 + data[5] -500)*100.0/1560.0*2*math.pi )
            else:
                print('Error Value! header1: ' + str(data[0]) + ', header2: ' + str(data[1]))
